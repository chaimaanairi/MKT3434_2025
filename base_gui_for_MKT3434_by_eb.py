...
class MLCourseGUI(QMainWindow):
    ...
    def create_data_section(self):
        ...
        self.dataset_combo.addItems(["Load Custom Dataset", "Iris Dataset", "Breast Cancer Dataset",
    ...
    def create_classical_ml_tab(self):
        ...
        self.loss_combo_reg = QComboBox()
        self.loss_combo_reg.addItems(["MSE", "MAE", "Huber Loss"])
        regression_layout.addWidget(QLabel("Loss Function (Regression):"))
        regression_layout.addWidget(self.loss_combo_reg)
        ...
        self.loss_combo_clf = QComboBox()
        self.loss_combo_clf.addItems(["Cross-Entropy", "Hinge Loss"])
        classification_layout.addWidget(QLabel("Loss Function (Classification):"))
        classification_layout.addWidget(self.loss_combo_clf)
        ...
        nb_group = self.create_algorithm_group("Naive Bayes", {"var_smoothing": "double", "Prior_Probabilities(Optional)": "double"})
        classification_layout.addWidget(nb_group)
    ...
    def train_model(self, model_name, param_widgets):
        ...
        try:
            ...
            if model_name == "Support Vector Machine":
                ...
                epsilon = param_widgets.get("epsilon", None)
                if epsilon:
                    self.current_model = SVR(kernel=kernel, C=C, epsilon=epsilon.value())
                else:
                    self.current_model = SVC(kernel=kernel, C=C)
            elif model_name == "Gaussian Naïve Bayes":
                var_smoothing = param_widgets["var_smoothing"].value()
                priors = None
                if "Prior_Probabilities(Optional)" in param_widgets:
                    prior_text = param_widgets["Prior_Probabilities(Optional)"].text()
                    if prior_text:
                        try:
                            priors = [float(p) for p in prior_text.split(',')]
                            priors = np.array(priors) / np.sum(priors)  # Normalize to sum to 1
                        except ValueError:
                            self.show_error("Invalid prior probabilities format. Use comma-separated values.")
                            return
            ...
            self.status_bar.showMessage(f"Model Trained: {model_name} | Loss: {loss:.4f}")
        except Exception as e:
